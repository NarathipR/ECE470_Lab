...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    end_loc_top=lab_invk(target_xw_yw_zw[0], target_xw_yw_zw[1], target_xw_yw_zw[2]+0.1,0)
    ...
    def yolo_m(self,img):
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                print(cls)
                bb=[]
                if classNames[cls] in obj_list:
                    bb=obj_list[classNames[cls]]
                    bb.append(box.xyxy[0].cpu().tolist())
                else:
                    bb.append(box.xyxy[0].cpu().tolist())
                obj_list[classNames[cls]]=bb
...
def estimate_place_position(pub_command,loop_rate,all_obj,target):
    ...
    for obj in all_obj:
        ...
        if pred+(y2-y1)<=ty2:
            ...
            target_xw_yw_zw=[x,y,0.035]
            move_block(pub_command, loop_rate, pt_y, target_xw_yw_zw, 4, 4)
        else:
            break
...
def main():
    ...
    #where to where
    target_obj="banana"
    if target_obj in obj_list_queue:
        #put in the position
        #use top and go down
        bb_obj=obj_list_queue[target_obj]
        for bb in bb_obj:
            #

    for pt_y in xw_yw_Y_queue:
        pt_y.append(0.030)
        if menu=='1':
            input_str=input("target position(tr,tl,br,bl):  ")
            target_xw_yw_zw=select_location(input_str)
        elif menu=='2':
            input_str=input("x y:  ")
            values=input_str.split()
            target_xw_yw_zw=[float(values[0]),float(values[1]),0.035]

        #values=input_str.split()
        move_block(pub_command, loop_rate, pt_y, target_xw_yw_zw, vel, accel)
    for pt_g in xw_yw_G_queue:
        pt_g.append(0.030)
        if menu=='1':
            input_str=input("target position(tr,tl,br,bl):  ")
            target_xw_yw_zw=select_location(input_str)
            ...
            values=input_str.split()
            target_xw_yw_zw=[float(values[0]),float(values[1]),0.035]
        move_block(pub_command, loop_rate, pt_g, target_xw_yw_zw, vel, accel)
    # # ========================= Student's code ends here ===========================
